...
function drawLorenzPointDual(p1, p2, p3) {
  ...
  const cx3 = Math.round(xOffset + (p3.x - centerX) * scale);
  const cy3 = Math.round(yOffset - (p3.z - centerZ) * scale);

  trace1.push({ x: cx1, y: cy1 });
  trace2.push({ x: cx2, y: cy2 });
  trace3.push({ x: cx3, y: cy3 });

  if (trace1.length > 1000) trace1.shift();
  if (trace2.length > 1000) trace2.shift();
  if (trace3.length > 1000) trace3.shift();
  ...
  for (let i = 0; i < trace3.length; i++) {
    if (i === 0) ctx.moveTo(trace3[i].x, trace3[i].y);
    else ctx.lineTo(trace3[i].x, trace3[i].y);
  }
  ctx.stroke();
...
function animate() {
  ...
  if (dualMode) {
    ...
    point3 = lorenzStep(point3, dt)
    simulationTime += dt;
    updateTimeDisplay();
    drawLorenzPointDual(point1, point2, point3);}
  } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("sigma").addEventListener("input", (e) => {
      ...
      if (!dualStarted) {
        ...
        point1 = { x: 1, y: 1, z: 1 };
        point2 = { x: 1.02, y: 1, z: 1 };
        point3 = { x: 0.009, y: 1, z: 1 };
        trace1 = [];
        trace2 = [];
        trace3 = [];
