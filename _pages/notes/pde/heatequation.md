...
\\[X(0) = 0 = A\text{cos}(\sqrt{-k}\times 0) + B\text{sin}(\sqrt{-k}\times 0)\\] this simplifies to \\[A\text{cos}(0) + B\text{sin}(0) = 0\\] We know that \\(\text{sin}(0) = 0\\) and that \\(\text{cos}(0) = 1\\). So our equation simplifies to: \\[A = 0\\] This tells us that \\(A\\) is \\(0\\), so now we can write \\[X(x) = Bsin(\sqrt{-k}x)\\] Since \\(A\\) being \\(0\\) cancels out the \\(cos\\) term. \
