...
Therefore, we need \\(-k\\) to cause \\(sin(-k)\\) to be \\(0\\) \\(\forall l\\) to satisfy our boundary condition. We know that \\(sin(x)) = 0\\) at integer multiples of \\(\pi\\). This tells us that if \\(-k = n\pi\\) \\(\forall n \in \mathbb{Z}^+\\)
