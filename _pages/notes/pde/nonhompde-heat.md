...
**Note:** After solving for the ODE in \\(T\\) we must also take into consideration the initial condition \\(\phi(x)\\) to solve for the uknown constant produced by the solution to the ODE in \\(T\\).
