...
\\[\text{A function } f \text{ defined on a a measurable subset } E \subset \mathbb{R}^d \text{ is measurable, if } \forall a \in \mathbb{R} \text{ the set } f^{-1}([-\infty, a])) = {x \in E: f(x) < a}\\]
